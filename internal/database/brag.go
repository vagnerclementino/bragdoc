...
func (r *sqliteBragRepository) SelectByCategory(ctx context.Context, userID int64, category domain.Category) ([]*domain.Brag, error) {
	// Get category ID from domain.Category
...
func (r *sqliteBragRepository) Insert(ctx context.Context, brag *domain.Brag) (*domain.Brag, error) {
	// Get category ID from domain.Category
	categoryID, err := r.getCategoryID(ctx, brag.Category)
	if err != nil {
		return nil, fmt.Errorf("failed to get category ID: %w", err)
	}
...
func (r *sqliteBragRepository) getCategoryID(ctx context.Context, category domain.Category) (int64, error) {
	// Try to get existing category by name
	existing, err := r.categoryRepo.GetByName(ctx, category.Name)
	if err == nil {
...
func (r *sqliteBragRepository) toDomainBrag(ctx context.Context, dbBrag *queries.Brag) (*domain.Brag, error) {
	...
	// Get category
	category, err := r.categoryRepo.Get(ctx, dbBrag.CategoryID)
	if err != nil {
